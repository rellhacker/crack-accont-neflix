...
if __name__ == "__main__":
    print('\n---------- Welcome To Cracked Accont NetFliX ----------\n')
    print('\n--------------⚠BYE FARREL CIREBON BLACK-HAT⚠-------------\n')
    ...
    email = input('Ketikan (crackedbyrellhack@gmail.com)').strip()
    for index, password in zip(range(password_data.__len__()), password_data):
        password = password.strip()
        if len(password) < MIN_PASSWORD_LENGTH:
            continue
        print("[OK] accont netflix [", index, "]: ", password)
